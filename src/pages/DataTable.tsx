...
import { Search, ChevronUp, ChevronDown, Eye, Edit, Trash2, Plus, Download } from "lucide-react";
...
const DataTable = () => {
  ...
  const departments = useMemo(() => ["All", ...Array.from(new Set(data.map((item) => item.department)))], [data]);
  const statuses: Array<"All" | Status> = ["All", "Active", "Inactive"];
  ...
  return (
    <div className="container-fluid py-4">
            ...
            <table className="table table-hover table-striped">
              <thead className="table-dark">
                <tr>
                  ...
                  <th className="cursor-pointer user-select-none" onClick={() => handleSort("name")}>
                    <div className="d-flex align-items-center">Name {getSortIcon("name")}</div>
                  </th>

                  <th className="cursor-pointer user-select-none" onClick={() => handleSort("email")}>
                    <div className="d-flex align-items-center">Email {getSortIcon("email")}</div>
                  </th>

                  <th className="cursor-pointer user-select-none" onClick={() => handleSort("role")}>
                    <div className="d-flex align-items-center">Role {getSortIcon("role")}</div>
                  </th>

                  <th className="cursor-pointer user-select-none" onClick={() => handleSort("department")}>
                    <div className="d-flex align-items-center">Department {getSortIcon("department")}</div>
                  </th>

                  <th className="cursor-pointer user-select-none" onClick={() => handleSort("status")}>
                    <div className="d-flex align-items-center">Status {getSortIcon("status")}</div>
                  </th>

                  <th className="cursor-pointer user-select-none" onClick={() => handleSort("joinDate")}>
                    <div className="d-flex align-items-center">Join Date {getSortIcon("joinDate")}</div>
                  </th>

                  <th className="cursor-pointer user-select-none" onClick={() => handleSort("salary")}>
                    <div className="d-flex align-items-center">Salary {getSortIcon("salary")}</div>
                  </th>
                  ...
                  <tr key={item.id} className={selectedRows.includes(item.id) ? "table-primary" : ""}>
                    ...
                    <td>
                      <div className="btn-group btn-group-sm">
                        ...
                        <button className="btn btn-outline-danger" title="Delete" onClick={() => handleDelete(item.id)}>
                          <Trash2 size={14} />
                        </button>
                      </div>
                    </td>
                ...
                <nav>
                  <ul className="pagination pagination-sm">
                    ...
                    {Array.from({ length: Math.min(5, totalPages) }, (_, i) => {
                      const base = Math.max(1, Math.min(totalPages - 4, currentPage - 2));
      ...
      {/* ✅ Vite/React: no `jsx` prop */}
      <style>{`
        .cursor-pointer { cursor: pointer; }
        .table th { border-top: none; font-weight: 600; }
        .table-hover tbody tr:hover { background-color: rgba(0, 123, 255, 0.075); }
