...
import { Search, ChevronRight, MessageCircle, Book, Zap, Users, ArrowRight, Star, CheckCircle } from 'lucide-react';
    ...
    <div className={`min-h-screen ${backgroundThemes[bgTheme]} position-relative overflow-hidden`}>
      ...
      <div className="position-fixed top-0 end-0 p-3 z-3">
        <div className="dropdown">
          <button
            className="btn btn-light btn-sm dropdown-toggle shadow-sm"
            type="button"
      ...
      {/* Hero Section */}
      <div className={`text-white py-20 position-relative z-2 ${
        ['animated', 'particles', 'waves', 'stars', 'matrix'].includes(bgTheme) ? 'bg-gradient-to-r from-blue-600 to-purple-600' :
        bgTheme === 'ocean' ? 'bg-gradient-to-r from-cyan-600 to-blue-600' :
        bgTheme === 'sunset' ? 'bg-gradient-to-r from-orange-600 to-pink-600' :
        bgTheme === 'forest' ? 'bg-gradient-to-r from-green-600 to-emerald-600' :
        bgTheme === 'cosmic' ? 'bg-gradient-to-r from-purple-600 to-indigo-600' :
        bgTheme === 'warm' ? 'bg-gradient-to-r from-yellow-600 to-orange-600' :
        bgTheme === 'cool' ? 'bg-gradient-to-r from-gray-600 to-slate-600' :
        bgTheme === 'minimal' ? 'bg-gradient-to-r from-gray-800 to-gray-900' :
        'bg-gradient-to-r from-blue-600 to-purple-600'
      }`}>
        <div className="container mx-auto px-4">
          <div className="text-center max-w-4xl mx-auto">
            <h1 className="display-4 fw-bold mb-4">How can we help you today?</h1>
            <p className="lead mb-5">Find answers, tutorials, and support resources to get the most out of our platform</p>
      ...
      <div className="container mx-auto px-4 py-5 position-relative z-2">
        {/* Quick Actions */}
              ...
              {quickActions.map((action, index) => (
                <div key={index} className="col-md-4">
                  <div className={`card border-0 shadow-sm h-100 hover-card ${
                    bgTheme === 'minimal' ? 'bg-gray-50' : `bg-${action.color} bg-opacity-10`
                  }`}>
                    <div className="card-body text-center p-4">
                      <div className={`rounded-circle bg-${action.color} bg-opacity-20 p-3 d-inline-flex mb-3`}>
        ...
        <div className="row">
                ...
                {categories.map((category) => (
                  <button
                    ...
                    className={`list-group-item list-group-item-action border-0 d-flex align-items-center ${
                      activeCategory === category.id ? `active bg-${category.color} bg-opacity-10` : ''
                    }`}
                    onClick={() => setActiveCategory(category.id)}
                  >
                    <category.icon 
                      size={20} 
                      className={`me-3 ${activeCategory === category.id ? `text-${category.color}` : 'text-muted'}`} 
                    />
          ...
          <div className="col-lg-9">
            ...
            <div className={`card border-0 shadow-sm mb-4 ${
              bgTheme === 'minimal' ? 'bg-white' : 'bg-white bg-opacity-90 backdrop-blur-sm'
            }`}>
              <div className="card-header bg-gradient-to-r from-gray-50 to-gray-100 border-0">
                <h4 className="mb-0">
                ...
                <div className="row g-3">
                  {helpArticles[activeCategory].map((article, index) => (
                    <div key={index} className="col-12">
                      <div className={`border rounded-3 p-4 hover-card ${
                        bgTheme === 'minimal' ? 'bg-gray-50' : 'bg-light bg-opacity-50'
                      }`}>
            ...
            <div className={`card border-0 shadow-sm ${
              ...
              <div className="card-header bg-gradient-to-r from-purple-50 to-blue-50 border-0">
      ...
      <div className={`text-white py-5 mt-5 ${
        bgTheme === 'ocean' ? 'bg-gradient-to-r from-cyan-800 to-blue-800' :
        bgTheme === 'sunset' ? 'bg-gradient-to-r from-orange-800 to-pink-800' :
        bgTheme === 'forest' ? 'bg-gradient-to-r from-green-800 to-emerald-800' :
        bgTheme === 'cosmic' ? 'bg-gradient-to-r from-purple-800 to-indigo-800' :
        bgTheme === 'warm' ? 'bg-gradient-to-r from-yellow-800 to-orange-800' :
        bgTheme === 'cool' ? 'bg-gradient-to-r from-gray-800 to-slate-800' :
        bgTheme === 'minimal' ? 'bg-gradient-to-r from-gray-900 to-black' :
